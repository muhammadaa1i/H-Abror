      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start lg:items-center">
          <div className="flex-1 flex flex-col justify-center mb-8 lg:mb-0">
            <address className="not-italic text-[#f8b830] text-base md:text-lg lg:text-xl xl:text-2xl font-semibold mb-12 leading-relaxed">
              {t('contacts.address')}
            </address>
            <div className="text-[#052035] space-y-2">
              <a href={`mailto:${t('contacts.email')}`} className="block underline hover:text-[#f8b830] transition-colors text-base md:text-lg xl:text-lg 2xl:text-xl">{t('contacts.email')}</a>
              <a href={`tel:${t('contacts.phone').replace(/\s+/g, '')}`} className="block font-bold text-lg md:text-xl xl:text-2xl 2xl:text-3xl hover:text-[#f8b830] transition-colors">{t('contacts.phone')}</a>
            </div>
          </div>
